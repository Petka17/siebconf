...
class GetRepoObjectIndex
  ...
  def perform config_id
    ...
    if repo_obj_index.size > 0
      ...
      puts_log "Update Siebel Configuration"
      siebel_configuration.status = "Getting Objects"
      siebel_configuration.upsert

      puts_log "Start Get Object Source Job"
      GetRepoObjects.perform_async config_id
    end

    puts_log "Get Object Index job is finished"
  end

  def get_repo_obj_index environment
    puts_log "Prepare variables for quering database"
    repo_obj_index = []
    ...
    puts_log "Last Sync Date: #{last_upd}"
    ...
    puts_log "#{user} #{passwd} #{url}"

    begin
      puts_log "Connect to database"
      conn = OracleConnection.create(user, passwd, url)
      select_stmt = conn.create_statement

      puts_log "Start quering each object type"
      object_type_list.each do |obj_type| 
        puts_log "#{obj_type[:name]}"
        ...
        puts_log "Condition: #{add_cond}, Column: #{column}, Versions: #{versions}"
